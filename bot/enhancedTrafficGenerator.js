...
class EnhancedTrafficGenerator {
  constructor() {
    ...
    this.connectionMetrics = new Map(); // Store connection metrics per domain
  }
  ...
  // Ping test untuk mengukur koneksi ke target URL
  async pingTest(url, timeout = 10000) {
  ...
  // Adaptive timeout berdasarkan ping test
  async getAdaptiveTimeout(targetUrl, sessionId) {
    const defaultTimeout = 60000; // 60 detik default
    
    try {
      this.log(sessionId, 'PING_TEST', `Testing connection to ${targetUrl}...`);
      
      const pingResult = await this.pingTest(targetUrl, 15000);
      
      if (pingResult.success) {
        // Klasifikasi response time
        let adaptiveTimeout;
        if (pingResult.responseTime < 1000) {
          adaptiveTimeout = 45000; // Cepat: 45 detik
          this.log(sessionId, 'PING_FAST', `Fast connection (${pingResult.responseTime}ms), using 45s timeout`);
        } else if (pingResult.responseTime < 3000) {
          adaptiveTimeout = 60000; // Sedang: 60 detik
          this.log(sessionId, 'PING_MEDIUM', `Medium connection (${pingResult.responseTime}ms), using 60s timeout`);
        } else if (pingResult.responseTime < 8000) {
          adaptiveTimeout = 90000; // Lambat: 90 detik
          this.log(sessionId, 'PING_SLOW', `Slow connection (${pingResult.responseTime}ms), using 90s timeout`);
        } else {
          adaptiveTimeout = 120000; // Sangat lambat: 120 detik
          this.log(sessionId, 'PING_VERY_SLOW', `Very slow connection (${pingResult.responseTime}ms), using 120s timeout`);
        }

        // Simpan metrics untuk domain ini
        this.connectionMetrics.set(targetUrl, {
          lastPing: pingResult.responseTime,
          lastCheck: Date.now(),
          recommendedTimeout: adaptiveTimeout
        });

        return adaptiveTimeout;
      } else {
        this.log(sessionId, 'PING_FAILED', `Ping test failed: ${pingResult.error}`);
        return 120000; // Fallback ke 120 detik jika ping gagal
      }
    } catch (error) {
      this.log(sessionId, 'PING_ERROR', `Ping test error: ${error.message}`);
  ...
  async startNewSession(config) {
    ...
    // Enhanced proxy management dengan free proxy option
    if (config.useFreeProxy !== false) {
      await this.addFreeProxies(sessionId);
    }
    ...
    // Execute session tanpa blocking
  ...
  // Add free proxies automatically
  async addFreeProxies(sessionId) {
    try {
      const freeProxies = [
        // Add some free proxy sources here
        'http://45.77.56.113:3128',
        'http://51.158.68.68:8811',
        'http://213.226.11.149:41878'
      ];
      
      const validFreeProxies = freeProxies.filter(proxy => 
        this.proxyHandler.validateProxyFormat(proxy) && 
        !this.proxyHandler.getAllProxies().includes(proxy)
      );
      
      if (validFreeProxies.length > 0) {
        this.proxyHandler.addMultipleProxies(validFreeProxies);
        this.log(sessionId, 'FREE_PROXIES_ADDED', `Added ${validFreeProxies.length} free proxies`);
      }
    } catch (error) {
      this.log(sessionId, 'FREE_PROXIES_ERROR', `Error adding free proxies: ${error.message}`);
    }
  }

  async executeSessionWithRetry(sessionId, config, retryCount = 0) {
    const maxRetries = config.maxRestarts || 2;
  ...
  async executeSession(sessionId, config) {
    ...
    let adaptiveTimeout = 60000;

    try {
      // Dapatkan adaptive timeout berdasarkan ping test
      adaptiveTimeout = await this.getAdaptiveTimeout(config.targetUrl, sessionId);
      ...
      const launchResult = await this.launchBrowserWithActiveProxy(config, sessionId, adaptiveTimeout);
      browser = launchResult.browser;
      currentProxy = launchResult.proxy;
      
      if (currentProxy) {
        this.proxyHandler.markProxyAsActive(currentProxy.original);
        this.log(sessionId, 'PROXY_SUCCESS', `Using proxy: ${currentProxy.url}`);
      } else {
      ...
      // Set timeout berdasarkan adaptive timeout
      ...
      // Enhanced request interception dengan timeout yang lebih longgar
      ...
      this.log(sessionId, 'STEP_3', `Navigating to: ${config.targetUrl} (Timeout: ${adaptiveTimeout/1000}s)`);
      
      try {
        // Enhanced navigation dengan retry mechanism
        const response = await this.enhancedNavigation(page, config.targetUrl, adaptiveTimeout, sessionId);
        ...
        await this.executeAllSteps(page, sessionId, config, adaptiveTimeout);
      ...
      if (currentProxy && (error.message.includes('proxy') || error.message.includes('timeout') || error.message.includes('NETWORK'))) {
        this.proxyHandler.markProxyAsFailed(currentProxy.original);
  ...
  // Enhanced navigation dengan retry logic
  async enhancedNavigation(page, url, timeout, sessionId, retryCount = 0) {
    const maxRetries = 2;
    ...
    try {
      ...
      if (retryCount < maxRetries) {
        const backoffTime = Math.pow(2, retryCount) * 5000; // 5, 10 detik
        this.log(sessionId, 'NAVIGATION_RETRY', 
          `Navigation failed, retrying in ${backoffTime/1000}s... (${retryCount + 1}/${maxRetries})`);
        
        await page.waitForTimeout(backoffTime);
        return this.enhancedNavigation(page, url, timeout, sessionId, retryCount + 1);
      } else {
        throw error;
      }
    }
  ...
  async launchBrowserWithActiveProxy(config, sessionId, timeout) {
    const args = [
      '--no-sandbox',
      ...
      `--user-agent="${new UserAgents({ deviceCategory: config.deviceType })}"`
    ];

    // Tambah args untuk handle timeout yang lebih baik
    args.push(
    ...
    const launchOptions = {
      ...
      timeout: timeout + 30000 // Browser launch timeout lebih lama dari navigation timeout
    };
  ...
  async executeAllSteps(page, sessionId, config, timeout) {
    const session = this.activeSessions.get(sessionId);
    const isOddSession = parseInt(sessionId.split('_')[1]) % 2 !== 0;
    
    // Adjust step timeouts berdasarkan overall timeout
    const baseStepTimeout = Math.min(timeout * 0.3, 30000); // Maksimal 30 detik per step
...
module.exports = EnhancedTrafficGenerator;